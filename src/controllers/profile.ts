...
export const getPublicProfile: RequestHandler = async (req, res) => {
  ...
  if (!user) return res.status(422).json({ error: "Invalid not found!" });
...
export const getPublicPlaylist: RequestHandler = async (req, res) => {
  ...
  if (!playlist) return res.status(422).json({ playlist: [] });
...
export const getPlaylistAudios: RequestHandler = async (req, res) => {
  ...
  const [playlistResult] = await Playlist.aggregate(aggregationLogin)

  if(!playlistResult){
    const [autoPlaylistResult] = await AutoGeneratedPlaylist.aggregate(aggregationLogin)
